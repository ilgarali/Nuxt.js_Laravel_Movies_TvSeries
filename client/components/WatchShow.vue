  ...
  <b-col cols="md-9" class="mt-4 bg-main" >
    ...
    <div class="tv-shows ml-4 mt-5 text-white">
      <h3 class="ml-1 font-weight-bold">
        Film izle:
        <span class="text-muted">{{tvshow.episode_name}}</span>
      </h3>
    </div>

    <div class="choose-language d-flex my-3 ml-2">
      <div class="text-white" v-for="(item, index) in tvshow.resource.dublaj" :key="index">
     
      </div>
      <div
        v-if="tvshow.resource.altyazi"
    ...
    <div class="main" v-if="tvshow.resource.altyazi">
      <div class="text-white"></div>
      <div class="subtitle">
        <div class="source d-flex ml-4 text-muted font-weight-bold">
          <div
            v-for="(item, index) in tvshow.resource.altyazi"
            :key="index"
            @click="link(index)"
        ...
        <b-container>
          <b-row>
            <b-col cols="md-12">
              <div
                class="video my-3"
                v-for="(item, index) in tvshow.resource"
                :key="index"
                @click="link(index)"
    ...
    <div class="dub" v-else>
       <div class="source d-flex ml-4 text-muted font-weight-bold">
          <div
            v-for="(item, index) in tvshow.resource.dublaj"
            :key="index"
            @click="link(index)"
            :class="[index < 1 ? 'active-source' : '']"
            class="border-source sources py-1 px-2 ml-1 mr-2"
...
export default {
  ...
  computed: {
   ...
   created() {
     setTimeout(async() => {
      ...
      await this.singleEpisode(this.$route.params.slug);
       this.loading = true
  ...
  methods: {
    ...
    ...mapActions({
      singleEpisode:"tvshow/singleEpisode"
    })
  },
};
</script>
